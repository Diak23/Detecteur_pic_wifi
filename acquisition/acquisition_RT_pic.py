...
def creer_dossier_acquisition():
    dossier_parent = "acquisitions"
    os.makedirs(dossier_parent, exist_ok=True)

    numero = 1
    while os.path.exists(f"{dossier_parent}/acquisition_{numero:03d}"):
        numero += 1

    dossier = f"{dossier_parent}/acquisition_{numero:03d}"
    os.makedirs(dossier)
...
def arreter():
    ...
    label_duree_pics.config(
        text=f"Durée moyenne des pics : {duree_moyenne_pics():.2f} s"
    )
...
def sauvegarder_csv():
    if dossier_acquisition is None:
        label_info.config(text="Erreur : démarre une acquisition d'abord.")
        return

    chemin = f"{dossier_acquisition}/mesures_wifi.csv"
    ...
    label_info.config(text=f"CSV sauvegardé : {chemin}")
...
def sauvegarder_csv_pics():
    ...
    pics = analyser_pics()
    chemin = f"{dossier_acquisition}/pics_wifi.csv"
    ...
    label_info.config(text=f"CSV des pics sauvegardé : {chemin}")
...
def sauvegarder_png():
    ...
    chemin = f"{dossier_acquisition}/graphe_wifi.png"
    fig.savefig(chemin, dpi=300)
    label_info.config(text=f"PNG sauvegardé : {chemin}")
...
def generer_rapport():
    ...
    chemin = f"{dossier_acquisition}/rapport_wifi.txt"
    ...
    with open(chemin, "w") as f:
        ...
        f.write(f"Dossier acquisition : {dossier_acquisition}\n")
        ...
        if len(pics) > 0:
            durees = [p["duree_s"] for p in pics]
            f.write(f"Durée moyenne des pics : {np.mean(durees):.2f} s\n")
            f.write(f"Durée max des pics : {np.max(durees):.2f} s\n")
            f.write(f"Durée min des pics : {np.min(durees):.2f} s\n")
        else:
