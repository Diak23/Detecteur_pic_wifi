...
def sauvegarder_csv():
    with open("mesures_wifi_reel_pic.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["temps_s", "rssi_dbm", "puissance_w"])
        for t, r, p in zip(temps, rssi, puissance):
            writer.writerow([t, r, p])

    label_info.config(text="CSV sauvegardé : mesures_wifi_reel_pic.csv")

def sauvegarder_png():
    fig.savefig("graphe_wifi_reel_pic.png", dpi=300)
    label_info.config(text="PNG sauvegardé : graphe_wifi_reel_pic.png")
...
def generer_rapport():
    ...
    with open("rapport_wifi_pic.txt", "w") as f:
    ...
    label_info.config(text="Rapport généré : rapport_wifi_pic.txt")
